...
export default function MTGCardSearch() {
  ...
  const [advancedFilters, setAdvancedFilters] = useState<AdvancedFilters>({
    priceRange: [0, 100], formats: [], keywords: [], customQuery: "", powerToughness: { power: { operator: "", value: "" }, toughness: { operator: "", value: "" } }, textContains: "", artist: "", flavorText: "", loyalty: { operator: "", value: "" }
  })
  ...
  const searchCards = async (
    ...
    try {
      ...
      const response = await fetch(`https://api.scryfall.com/cards/search?q=${encodedQuery}&order=name&page=${apiPageToFetch}`)
      if (response.ok) {
  ...
  useEffect(() => {
    ...
    searchCards("", { editions: [], color: "all", rarity: "all", type: "all", cmc: "all" }, { priceRange: [0, 100], formats: [], keywords: [], customQuery: "", powerToughness: { power: { operator: "", value: "" }, toughness: { operator: "", value: "" } }, textContains: "", artist: "", flavorText: "", loyalty: { operator: "", value: "" } }, 1)
  }, [])
